...
* ``yaw``, ``pitch`` and ``roll`` describe the rectilinear camera pose in the
  VR180/world coordinate system.  Positive yaw turns right and positive pitch
  turns up.  Roll is around the camera's local optical axis.
...
_GRID_CACHE_MAX_ENTRY_BYTES = 64 * 1024 * 1024
...
def _rotation_camera_to_world(
    ...
    *,
    ...
    """Return camera-to-world pose with yaw/right, pitch/up and local roll.

    Column-vector form is ``Ry(yaw) @ Rx(-pitch) @ Rz(roll)``.  The negative
    angle on the conventional X rotation is what makes positive pitch move the
    optical axis toward +Y in the established +X-right, +Y-up, +Z-forward
    basis.  Putting roll on the right makes it local: roll cannot move the
    optical axis itself.
    """
    ...
    rx_negative = torch.tensor(
        [[1.0, 0.0, 0.0], [0.0, cp, sp], [0.0, -sp, cp]], dtype=dtype
    )
    rz = torch.tensor([[cr, -sr, 0.0], [sr, cr, 0.0], [0.0, 0.0, 1.0]], dtype=dtype)
    return ry @ rx_negative @ rz


def _grid_storage_bytes(output_height: int, output_width: int) -> int:
    """Bytes retained by a float32 XY grid plus its bool known mask."""

    return int(output_height) * int(output_width) * (2 * 4 + 1)


def _cacheable_grid(output_height: int, output_width: int) -> bool:
    return _grid_storage_bytes(output_height, output_width) <= _GRID_CACHE_MAX_ENTRY_BYTES
...
def _rect_to_vr_grid_uncached(
    ...
    source_x_limit = 1.0 - 1.0 / float(input_width)
    source_y_limit = 1.0 - 1.0 / float(input_height)
    ...
    for row_start in range(0, output_height, _GRID_BUILD_ROWS):
        ...
        valid = (
            (z > 1.0e-8)
            & (grid_x >= -source_x_limit)
            & (grid_x <= source_x_limit)
            & (grid_y >= -source_y_limit)
            & (grid_y <= source_y_limit)
        )
...
def _cached_rect_to_vr_grid_small(
    input_height: int,
    input_width: int,
    output_height: int,
    output_width: int,
...
def _cached_rect_to_vr_grid(
    ...
    if not _cacheable_grid(output_height, output_width):
        return _rect_to_vr_grid_uncached(*arguments)
    return _cached_rect_to_vr_grid_small(*arguments)
...
@lru_cache(maxsize=_GRID_CACHE_ENTRIES)
def _cached_vr_to_rect_grid_small(
    input_height: int,
    input_width: int,
    output_height: int,
    output_width: int,
...
def _cached_vr_to_rect_grid(
    ...
    if not _cacheable_grid(output_height, output_width):
        return _vr_to_rect_grid_uncached(*arguments)
    return _cached_vr_to_rect_grid_small(*arguments)
...
def _supports_float32_grid_with_low_precision_input(
    device_type: str,
    device_index: Optional[int],
    image_dtype: torch.dtype,
) -> bool:
    """Probe whether grid_sample accepts a float32 grid with this input dtype."""

    device = torch.device(device_type, device_index)
    try:
        probe_image = torch.zeros((1, 1, 2, 2), device=device, dtype=image_dtype)
        probe_grid = torch.zeros((1, 1, 1, 2), device=device, dtype=torch.float32)
        F.grid_sample(
            probe_image,
            probe_grid,
            mode="bilinear",
...
def _sample_with_grid(
    ...
    *,
    ...
    if image.dtype in (torch.float16, torch.bfloat16):
        mixed_supported = _supports_float32_grid_with_low_precision_input(
            image.device.type, image.device.index, image.dtype
        )
        sampling_image = image if mixed_supported else image.float()
        grid_dtype = torch.float32
    else:
        sampling_image = image
        grid_dtype = image.dtype
    sampled_parts = []
    mask_parts = []
    for start in range(0, output_height, rows):
        stop = min(start + rows, output_height)
        grid = grid_cpu[start:stop].to(device=image.device, dtype=grid_dtype)
...
def rectilinear_to_vr180(
    ...
    *,
    ...
    grid, known_cpu = _cached_rect_to_vr_grid(
        int(image.shape[2]),
        int(image.shape[3]),
        output_height,
        output_width,
...
def vr180_to_rectilinear(
    ...
    *,
    ...
    if mode not in {"bilinear", "nearest", "bicubic"}:
        raise ValueError("mode must be bilinear, nearest or bicubic")
    grid, geometric_known_cpu = _cached_vr_to_rect_grid(
        int(image.shape[2]),
        int(image.shape[3]),
        output_height,
        output_width,
...
def clear_projection_cache() -> None:
    _cached_rect_to_vr_grid_small.cache_clear()
    _cached_vr_to_rect_grid_small.cache_clear()
    _supports_float32_grid_with_low_precision_input.cache_clear()


def projection_cache_info() -> dict:
    """Return JSON-friendly cache diagnostics for tests and operational logs."""

    forward = _cached_rect_to_vr_grid_small.cache_info()
    reverse = _cached_vr_to_rect_grid_small.cache_info()
    return {
        "max_entries_per_direction": _GRID_CACHE_ENTRIES,
        "max_entry_bytes": _GRID_CACHE_MAX_ENTRY_BYTES,
        "max_retained_bytes_per_direction": (
            _GRID_CACHE_ENTRIES * _GRID_CACHE_MAX_ENTRY_BYTES
        ),
